...
pub struct RuleSpec {
    ...
    ///
    /// When true, `self.bind(on_x=self.handler)` prefers the handler
    /// on the same class as the bind() call.  Off by default because
    /// the lookup leaks into every language — in Clojure the enclosing
...
pub const SPECS: &[RuleSpec] = &[
    ...
    RuleSpec {
        ...
        gap: "",
        observer_types: NONE,
        handler_on_enclosing_owner: false,
    },
    RuleSpec {
        id: "flask",
        language: "python",
    ...
    RuleSpec {
        ...
        detect_calls: NONE,
        attributes: NONE,
        registrars: &["bind", "fbind", "schedule_once", "schedule_interval"],
        base_types: &[
            "App",
            "Widget",
            "Popup",
        ...
        methods: &[
            ...
            "on_scroll_stop",
            "on_keyboard_down",
            // Inherited property observers — built-in properties from
            // Kivy base widgets that subclasses commonly override.
            "on_text",
            "on_state",
            "on_active",
            "on_press",
        ...
        handler_on_enclosing_owner: true,
    },
    // ---------------------------------------------------------------
    ...
    RuleSpec {
        ...
        detect_calls: NONE,
        attributes: NONE,
        // Receiver-qualified: a bare `on`/`handle` would claim every
        // jQuery and DOM listener in the app. `app.on('ready', …)` is
        // the lifecycle entry; the channel or event is the route.
        registrars: &[
            "ipcMain.handle",
            "ipcMain.handleOnce",
            "ipcMain.on",
    ...
    RuleSpec {
        ...
        // Padrino apps are loaded by Bundler, so no `require "padrino"` appears; `config/apps.rb` mounts them and is Padrino's own convention.
        detect_paths: &["config/apps.rb"],
        detect_calls: NONE,
        attributes: NONE,
        registrars: NONE,
        base_types: NONE,
    ...
    RuleSpec {
        ...
        string_targets: false,
        node: true,
        gap: "cgg has no entry rules for Oak (Deno). Handlers are middleware callbacks -- `router.get('/books/:id', (ctx) => {...})`, `router.post(...)`, `app.use(async (ctx, next) => {...})`, plus the mounting calls `app.use(router.routes())` and `app.use(router.allowedMethods())` -- and `get` / `use` are too ambiguous to claim without corroboration. Detection covers the URL, JSR and bare-specifier forms, version-pinned or not (`https://deno.land/x/oak@v12.6.1/mod.ts`, `jsr:@oak/oak@^17`), including a re-export through a `deps.ts`. Grep for `new Router(`, `.routes()` and `app.use(` and read the surrounding callbacks; also check `app.addEventListener('error', ...)` and `app.listen(...)`.",
        observer_types: NONE,
        handler_on_enclosing_owner: false,
    },
    RuleSpec {
    ...
    RuleSpec {
        ...
        kind: TrustKind::Lifecycle,
        detect: &["puma"],
        // `config/puma.rb` is Puma's configuration DSL and requires nothing — Puma loads it.
        detect_paths: &["config/puma.rb"],
        detect_calls: NONE,
        attributes: NONE,
        registrars: NONE,
        base_types: NONE,
...
pub fn builtin() -> Vec<FrameworkRule> {
    SPECS
        ...
        .map(|s| FrameworkRule {
            ...
            handler_on_enclosing_owner: s.handler_on_enclosing_owner,
        })
        .collect()
}
