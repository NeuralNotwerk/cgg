...
impl<'a> VerilogWalker<'a> {
    ...
    fn first_simple_identifier(&self, node: Node) -> Option<String> {
        ...
        if c.goto_first_child() {
            loop {
                if let Some(s) = self.first_simple_identifier(c.node()) {
                    return Some(s);
                }
    ...
    fn walk(&mut self, node: Node) {
        match node.kind() {
            ...
            "task_declaration" | "function_declaration" => {
                let id_kind = if node.kind() == "task_declaration" {
                    "task_identifier"
                } else {
                    "function_identifier"
                };
                ...
                if !name.is_empty() {
                    ...
                    self.facts.definitions.push(DefRecord {
                        ...
                        signature_hint: super::extract_signature(self.text(node)),
                        visibility: String::new(),
                        attributes: vec![if node.kind() == "task_declaration" {
                            "task".into()
                        } else {
                            "function".into()
                        }],
...
mod tests {
    ...
    fn extract(src: &str) -> FileFacts {
        ...
        p.set_language(&tree_sitter_verilog::LANGUAGE.into())
            .unwrap();
    ...
    fn system_call_captured() {
        ...
        let f = extract(src);
        assert!(
            f.references.iter().any(|r| r.name == "$display"),
            "refs: {:?}",
            f.references
        );
    }
