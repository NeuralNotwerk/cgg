...
impl LanguagePlugin for ObjcPlugin {
    ...
    fn extract(&self, file: FileId, path: &Path, tree: &Tree, source: &[u8]) -> FileFacts {
        ...
        let mut w = ObjcWalker { source, facts: &mut facts, scope: Vec::new() };
        w.walk(tree.root_node());
        facts
    }
}

struct ObjcWalker<'a> { source: &'a [u8], facts: &'a mut FileFacts, scope: Vec<String> }

impl<'a> ObjcWalker<'a> {
    fn text(&self, n: Node) -> &str { n.utf8_text(self.source).unwrap_or("") }
    fn qn(&self, s: &str) -> String {
        if self.scope.is_empty() { s.to_string() } else { format!("{}::{s}", self.scope.join("::")) }
    }

    fn walk(&mut self, node: Node) {
        match node.kind() {
            "class_implementation" | "class_interface" => {
                let name = node.child(1) // class name is second child (after @implementation/@interface)
    ...
    fn record_method(&mut self, node: Node) {
        ...
        let variant = if name == "init" { DefVariant::Constructor }
            else if is_class { DefVariant::StaticMethod }
...
mod tests {
    ...
    fn extract(src: &str) -> FileFacts {
        ...
        ObjcPlugin.extract(FileId::new(0), &PathBuf::from("/tmp/__cgg_test__/x.m"), &tree, src.as_bytes())
    }
    ...
    fn class_methods() {
        ...
        let f = extract(src);
        assert!(f.definitions.iter().any(|d| d.simple_name == "run" && d.variant == DefVariant::InherentMethod));
    ...
    fn message_sends() {
        ...
        let f = extract(src);
        assert!(f.references.iter().any(|r| r.name == "run" && r.receiver_hint == "self"));
        assert!(f.references.iter().any(|r| r.name == "create" && r.receiver_hint == "Helper"));
    }
}
