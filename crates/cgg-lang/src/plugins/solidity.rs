...
impl LanguagePlugin for SolidityPlugin {
    fn id(&self) -> &'static str { "solidity" }
    fn extensions(&self) -> &'static [&'static str] { &[".sol"] }
    fn signals(&self) -> crate::PluginSignals {
        crate::PluginSignals { visibility: true, ..Default::default() }
    }

    fn ts_language(&self) -> tree_sitter::Language { tree_sitter_solidity::LANGUAGE.into() }
    ...
    fn extract(&self, file: FileId, path: &Path, tree: &Tree, source: &[u8]) -> FileFacts {
        ...
        let mut w = SolidityWalker { source, facts: &mut facts, scope: Vec::new() };
...
impl<'a> SolidityWalker<'a> {
    ...
    fn walk(&mut self, node: Node) {
        match node.kind() {
            "contract_declaration" | "library_declaration" | "interface_declaration" => {
                let name = self.child_kind(node, "identifier").map(|n| self.text(n).to_string()).unwrap_or_default();
                if !name.is_empty() { self.scope.push(name); self.walk_children(node); self.scope.pop(); }
            ...
            "import_directive" => {
                if let Some(s) = self.child_kind(node, "string") {
                    let raw = self.text(s);
                    let path = raw.trim_matches(|c: char| c == '"' || c == '\'').to_string();
                    if !path.is_empty() {
            ...
            "call_expression" => { self.record_call(node); self.walk_children(node); return; }
            _ => {}
        }
        self.walk_children(node);
    }
    ...
    fn record_function(&mut self, node: Node, is_ctor: bool) {
        let name = if is_ctor {
            ...
            self.child_kind(node, "identifier").map(|n| self.text(n).to_string()).unwrap_or_default()
        };
        if name.is_empty() { return; }
        let variant = if is_ctor { DefVariant::Constructor }
            else if self.scope.is_empty() { DefVariant::FreeFunction }
            else { DefVariant::InherentMethod };
        let qn = self.qn(&name);
        let (sl, el) = ((node.start_position().row as u32) + 1, (node.end_position().row as u32) + 1);
        let visibility = self.child_kind(node, "visibility").map(|n| self.text(n).to_string()).unwrap_or_default();
    ...
    fn record_call(&mut self, node: Node) {
        ...
        let inner = if callee.kind() == "expression" { callee.child(0).unwrap_or(callee) } else { callee };
        let (name, receiver) = match inner.kind() {
            ...
            "member_expression" => {
                ...
                let mut idents: Vec<Node> = inner.children(&mut inner.walk())
                    .filter(|c| c.kind() == "identifier" || c.kind() == "expression")
                    .collect();
                let property = idents.pop()
                    .map(|n| {
                        if n.kind() == "expression" {
                            n.child(0).map(|c| self.text(c).to_string()).unwrap_or_default()
                        } else { self.text(n).to_string() }
                    })
                    .unwrap_or_default();
                let object = idents.into_iter().next()
                    .map(|n| {
                        if n.kind() == "expression" {
                            n.child(0).map(|c| self.text(c).to_string()).unwrap_or_default()
                        } else { self.text(n).to_string() }
...
mod tests {
    ...
    fn extract(src: &str) -> FileFacts {
        ...
        p.set_language(&tree_sitter_solidity::LANGUAGE.into()).unwrap();
        let tree = p.parse(src, None).unwrap();
        SolidityPlugin.extract(FileId::new(0), &PathBuf::from("/tmp/x.sol"), &tree, src.as_bytes())
    }
    ...
    fn contract_methods_and_ctor() {
        ...
        assert_eq!(by.get("Token.constructor"), Some(&DefVariant::Constructor), "by: {by:?}");
    ...
    fn library_function_is_free() {
        ...
        assert!(f.definitions.iter().any(|d| d.qualified_name == "M.add"), "defs: {:?}", f.definitions);
    }
    ...
    fn import_captured() {
        ...
        let f = extract(src);
        assert!(f.imports.iter().any(|i| i.path == "./Other.sol"), "imports: {:?}", f.imports);
    }
    ...
    fn call_captured() {
        ...
        let f = extract(src);
        assert!(f.references.iter().any(|r| r.name == "g"), "refs: {:?}", f.references);
        assert!(f.references.iter().any(|r| r.name == "k" && r.receiver_hint == "h"), "refs: {:?}", f.references);
    }
}
