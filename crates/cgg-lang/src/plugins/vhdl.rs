...
impl LanguagePlugin for VhdlPlugin {
    ...
    fn extract(&self, file: FileId, path: &Path, tree: &Tree, source: &[u8]) -> FileFacts {
        ...
        let mut w = VhdlWalker { source, facts: &mut facts, scope: Vec::new() };
        w.walk(tree.root_node());
        facts
    }
}

struct VhdlWalker<'a> { source: &'a [u8], facts: &'a mut FileFacts, scope: Vec<String> }
...
impl<'a> VhdlWalker<'a> {
    ...
    fn walk(&mut self, node: Node) {
        match node.kind() {
            "entity_declaration" => { self.record_named(node, "entity", DefVariant::FreeFunction); self.walk_children(node); return; }
            ...
            "subprogram_definition" => { self.record_subprogram(node); self.walk_children(node); return; }
            "library_clause" => {
                if let Some(list) = self.child_kind(node, "logical_name_list") {
                    let names: Vec<String> = self.text(list).split(',')
                        .map(|s| s.trim().to_string())
                        .filter(|s| !s.is_empty())
                        .collect();
                    for nm in names {
                        self.facts.imports.push(ImportRecord {
                            kind: "library".into(), path: nm, alias: String::new(),
                            site_line: (node.start_position().row as u32) + 1,
                            site_byte: node.start_byte() as u32,
                        });
                    }
            ...
            "use_clause" => {
                if let Some(list) = self.child_kind(node, "selected_name_list") {
                    ...
                    if !text.is_empty() {
                        self.facts.imports.push(ImportRecord {
                            kind: "use".into(), path: text, alias: String::new(),
                            site_line: (node.start_position().row as u32) + 1,
                            site_byte: node.start_byte() as u32,
                        });
                    }
    ...
    fn record_named(&mut self, node: Node, tag: &str, variant: DefVariant) {
        ...
        let (sl, el) = ((node.start_position().row as u32) + 1, (node.end_position().row as u32) + 1);
        self.facts.definitions.push(DefRecord {
            simple_name: name.clone(), qualified_name: self.qn(&name), variant,
            start_line: sl, end_line: el,
            start_byte: node.start_byte() as u32, end_byte: node.end_byte() as u32,
            signature_hint: super::extract_signature(self.text(node)),
    ...
    fn record_subprogram(&mut self, node: Node) {
        ...
        let spec = self.child_kind(node, "procedure_specification")
            .or_else(|| self.child_kind(node, "function_specification"));
        let Some(spec) = spec else { return };
        let name = self.child_kind(spec, "identifier").map(|n| self.text(n).to_string()).unwrap_or_default();
        if name.is_empty() { return; }
...
mod tests {
    ...
    fn extract(src: &str) -> FileFacts {
        ...
        VhdlPlugin.extract(FileId::new(0), &PathBuf::from("/tmp/x.vhd"), &tree, src.as_bytes())
    }
    ...
    fn entity_and_architecture() {
        ...
        assert!(f.references.iter().any(|r| r.name == "counter"), "refs: {:?}", f.references);
    }
    ...
    fn library_and_use_are_imports() {
        ...
        let f = extract(src);
        assert!(f.imports.iter().any(|i| i.path == "ieee" && i.kind == "library"));
        assert!(f.imports.iter().any(|i| i.path.contains("std_logic_1164") && i.kind == "use"));
    }
}
