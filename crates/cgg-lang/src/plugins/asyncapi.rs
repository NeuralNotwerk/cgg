...
impl LanguagePlugin for AsyncApiPlugin {
    ...
    fn extract(&self, file: FileId, path: &Path, tree: &Tree, source: &[u8]) -> FileFacts {
        ...
        structured::add_section_defs(top, &["operations"], "operation", source, &mut facts);
...
mod tests {
    ...
    fn extract(src: &str) -> FileFacts {
        ...
        AsyncApiPlugin.extract(FileId::new(0), &PathBuf::from("/tmp/__cgg_test__/async.yaml"), &tree, src.as_bytes())
    }
...
"#;
    ...
    fn extracts_channel_message_schema_defs() {
        ...
        assert!(names.contains(&"light/measured"), "channel def, got {names:?}");
    ...
    fn channel_to_message_and_message_to_schema_refs() {
        ...
        assert!(refs.contains(&"LightMeasured"), "channel -> message, got {refs:?}");
    ...
    fn channel_ref_falls_inside_channel_def() {
        let f = extract(STREETLIGHTS);
        let chan = f.definitions.iter().find(|d| d.simple_name == "light/measured").unwrap();
        let msg_ref = f.references.iter().find(|r| r.name == "LightMeasured").unwrap();
        assert!(
