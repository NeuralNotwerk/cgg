...
impl<'a> ZigWalker<'a> {
    ...
    fn walk(&mut self, node: Node) {
        match node.kind() {
            ...
            "builtin_function" => {
                // @import("std") -> import
                let bi = node.children(&mut node.walk())
                    .find(|c| c.kind() == "builtin_identifier")
                    .map(|n| self.text(n).to_string()).unwrap_or_default();
                if bi == "@import" {
                    let arg = node.children(&mut node.walk())
                        .find(|c| c.kind() == "arguments")
                        .and_then(|a| a.children(&mut a.walk()).find(|c| c.kind() == "string"))
                        .map(|n| self.text(n).trim_matches('"').to_string())
                        .unwrap_or_default();
                    if !arg.is_empty() {
                        self.facts.imports.push(ImportRecord {
                            kind: "import".into(), path: arg, alias: String::new(),
                            site_line: (node.start_position().row as u32)+1,
                            site_byte: node.start_byte() as u32,
                        });
                    }
...
mod tests {
    ...
    fn extract(src: &str) -> FileFacts {
        ...
        ZigPlugin.extract(FileId::new(0), &PathBuf::from("/tmp/__cgg_test__/x.zig"), &tree, src.as_bytes())
    }

    #[test]
    fn functions_extracted() {
        let f = extract("fn add(a: i32, b: i32) i32 { return a + b; }\npub fn main() !void {}\n");
