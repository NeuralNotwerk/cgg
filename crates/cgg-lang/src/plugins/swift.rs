...
impl LanguagePlugin for SwiftPlugin {
    fn id(&self) -> &'static str { "swift" }
    fn extensions(&self) -> &'static [&'static str] { &[".swift"] }
    fn resolver_kind(&self) -> ResolverKind { ResolverKind::Custom }
    fn ts_language(&self) -> tree_sitter::Language { tree_sitter_swift::LANGUAGE.into() }
    ...
    fn extract(&self, file: FileId, path: &Path, tree: &Tree, source: &[u8]) -> FileFacts {
        ...
        let mut w = SwiftWalker { source, facts: &mut facts, scope: Vec::new() };
...
impl<'a> SwiftWalker<'a> {
        ...
        match node.kind() {
            ...
            "init_declaration" => {
                ...
                self.facts.definitions.push(DefRecord {
                    simple_name: "init".into(), qualified_name: qn,
    ...
    fn record_function(&mut self, node: Node) {
        ...
        let variant = if is_static { DefVariant::StaticMethod }
        ...
        let (name, recv) = match callee.kind() {
            ...
            "navigation_expression" => {
                let target = callee.child_by_field_name("target")
                    .map(|n| self.text(n).to_string()).unwrap_or_default();
                let suffix = callee.children(&mut callee.walk())
                    .find(|c| c.kind() == "navigation_suffix")
                    .and_then(|s| s.children(&mut s.walk()).find(|c| c.kind() == "simple_identifier"))
                    .map(|n| self.text(n).to_string()).unwrap_or_default();
                (suffix, target)
            }
            _ => return,
...
mod tests {
    ...
    fn extract(src: &str) -> FileFacts {
        ...
        SwiftPlugin.extract(FileId::new(0), &PathBuf::from("/tmp/__cgg_test__/x.swift"), &tree, src.as_bytes())
    }
    ...
    fn free_function() {
        ...
        let f = extract(src);
        assert!(f.definitions.iter().any(|d| d.simple_name == "greet" && d.variant == DefVariant::FreeFunction));
    }
