...
impl LanguagePlugin for CmakePlugin {
    ...
    fn extract(&self, file: FileId, path: &Path, tree: &Tree, source: &[u8]) -> FileFacts {
        ...
        let mut w = CmakeWalker { source, facts: &mut facts };
        w.walk(tree.root_node());
        facts
    }
}

struct CmakeWalker<'a> { source: &'a [u8], facts: &'a mut FileFacts }
...
impl<'a> CmakeWalker<'a> {
    ...
    fn walk(&mut self, node: Node) {
        match node.kind() {
            "function_def" => { self.record_block(node, "function_command"); self.walk_children(node); return; }
            "macro_def" => { self.record_block(node, "macro_command"); self.walk_children(node); return; }
            "normal_command" => { self.record_call(node); self.walk_children(node); return; }
            _ => {}
        }
    ...
    fn record_block(&mut self, node: Node, header_kind: &str) {
        ...
        let args = header.children(&mut header.walk()).find(|c| c.kind() == "argument_list");
        let Some(args) = args else { return };
        let name = args.children(&mut args.walk())
    ...
    fn record_call(&mut self, node: Node) {
        let Some(ident) = node.children(&mut node.walk()).find(|c| c.kind() == "identifier") else { return };
        let name_raw = self.text(ident);
        let name = name_raw.to_string();
        let lower = name_raw.to_ascii_lowercase();
        if name.is_empty() { return; }

        if matches!(lower.as_str(), "include" | "add_subdirectory" | "find_package") {
            if let Some(args) = node.children(&mut node.walk()).find(|c| c.kind() == "argument_list") {
                if let Some(first) = args.children(&mut args.walk()).find(|c| c.kind() == "argument") {
                    let path = self.text(first).trim_matches(|c: char| c == '"' || c == '\'').to_string();
                    if !path.is_empty() {
                        self.facts.imports.push(ImportRecord {
                            kind: lower, path, alias: String::new(),
...
mod tests {
    ...
    fn extract(src: &str) -> FileFacts {
        ...
        CmakePlugin.extract(FileId::new(0), &PathBuf::from("/tmp/x.cmake"), &tree, src.as_bytes())
    }
    ...
    fn include_is_import() {
        ...
        let f = extract(src);
        assert!(f.imports.iter().any(|i| i.path == "FetchContent" && i.kind == "include"));
        assert!(f.imports.iter().any(|i| i.path == "deps/foo" && i.kind == "add_subdirectory"));
    }
}
