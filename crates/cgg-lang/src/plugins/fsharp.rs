...
impl LanguagePlugin for FsharpPlugin {
    fn id(&self) -> &'static str { "fsharp" }
    fn extensions(&self) -> &'static [&'static str] { &[".fs", ".fsi", ".fsx"] }
    fn resolver_kind(&self) -> ResolverKind { ResolverKind::Custom }
    fn ts_language(&self) -> tree_sitter::Language { tree_sitter_fsharp::LANGUAGE_FSHARP.into() }
    ...
    fn extract(&self, file: FileId, path: &Path, tree: &Tree, source: &[u8]) -> FileFacts {
        ...
        let mut w = FsharpWalker { source, facts: &mut facts, scope: Vec::new() };
...
impl<'a> FsharpWalker<'a> {
    ...
    fn walk(&mut self, node: Node) {
        match node.kind() {
            "anon_type_defn" => {
                let name = self.child_kind(node, "type_name")
            ...
            "function_or_value_defn" => { self.record_let(node); self.walk_children(node); return; }
            "member_defn" => { self.record_member(node); self.walk_children(node); return; }
            "import_decl" => {
                if let Some(li) = self.child_kind(node, "long_identifier") {
                    let path = self.text(li).to_string();
                    if !path.is_empty() {
                        self.facts.imports.push(ImportRecord {
                            kind: "open".into(), path, alias: String::new(),
            ...
            "application_expression" => { self.record_call(node); self.walk_children(node); return; }
            _ => {}
        }
    ...
    fn record_let(&mut self, node: Node) {
        ...
        let Some(decl) = self.child_kind(node, "function_declaration_left") else { return };
        let Some(ident) = self.child_kind(decl, "identifier") else { return };
    ...
    fn record_member(&mut self, node: Node) {
        let Some(method) = self.child_kind(node, "method_or_prop_defn") else { return };
        let Some(prop) = self.child_kind(method, "property_or_ident") else { return };
        // property_or_ident contains identifier(s); we want the last one (after `this.`)
        let mut last_ident: Option<Node> = None;
        let mut c = prop.walk();
        if c.goto_first_child() {
            loop { if c.node().kind() == "identifier" { last_ident = Some(c.node()); } if !c.goto_next_sibling() { break; } }
        }
    ...
    fn record_call(&mut self, node: Node) {
        ...
        loop {
            if let Some(first) = cur.child(0) {
                if first.kind() == "application_expression" { cur = first; continue; }
                let inner = if first.kind() == "long_identifier_or_op" { first.child(0).unwrap_or(first) } else { first };
                match inner.kind() {
                    "long_identifier" => {
                        ...
                        if c.goto_first_child() {
                            loop { if c.node().kind() == "identifier" { idents.push(self.text(c.node()).to_string()); } if !c.goto_next_sibling() { break; } }
                        }
...
mod tests {
    ...
    fn extract(src: &str) -> FileFacts {
        ...
        p.set_language(&tree_sitter_fsharp::LANGUAGE_FSHARP.into()).unwrap();
        let tree = p.parse(src, None).unwrap();
        FsharpPlugin.extract(FileId::new(0), &PathBuf::from("/tmp/x.fs"), &tree, src.as_bytes())
    }
    ...
    fn type_members() {
        ...
        let f = extract(src);
        let qns: Vec<_> = f.definitions.iter().map(|d| d.qualified_name.as_str()).collect();
    ...
    fn application_captured() {
        ...
        assert!(f.references.iter().any(|r| r.name == "Run" && r.receiver_hint == "s"),
            "refs: {:?}", f.references);
    }
}
