...
impl<'a> PowerShellWalker<'a> {
    ...
    fn record_function(&mut self, node: Node) {
        let Some(name_node) = self.child_kind(node, "function_name") else {
            return;
        };
    ...
    fn record_method(&mut self, node: Node) {
        ...
        if c.goto_first_child() {
            loop {
                ...
                match n.kind() {
                    "class_attribute" => {
                        if self.text(n).contains("static") {
                            is_static = true;
                        }
                    }
    ...
    fn record_command(&mut self, node: Node) {
        ...
        let name_node = self.child_kind(node, "command_name").or_else(|| {
            self.child_kind(node, "command_name_expr")
    ...
    fn record_using(&mut self, node: Node) {
        ...
        let Some(elements) = self.child_kind(node, "command_elements") else {
            return;
        };
    ...
    fn record_invokation(&mut self, node: Node) {
        ...
        if c.goto_first_child() {
            loop {
                ...
                match n.kind() {
                    "variable" => {
                        receiver = self.text(n).trim_start_matches('$').to_string();
                        break;
                    }
                    "type_literal" => {
...
mod tests {
    ...
    fn extract(src: &str) -> FileFacts {
        ...
        p.set_language(&tree_sitter_powershell::LANGUAGE.into())
            .unwrap();
    ...
    fn free_function() {
        let src =
            "function Get-Greeting { param([string]$Name) Write-Host \"hi $Name\" }\n";
        let f = extract(src);
        assert!(
            f.definitions.iter().any(|d| d.simple_name == "Get-Greeting"
                && d.variant == DefVariant::FreeFunction),
            "defs: {:?}",
            f.definitions
        );
    }
...
";
    ...
    fn cmdlet_call_captured() {
        ...
        let f = extract(src);
        assert!(
            f.references.iter().any(|r| r.name == "Get-Greeting"),
            "refs: {:?}",
            f.references
        );
    }
    ...
    fn method_invokation_captured() {
        ...
        assert!(
            f.references
                .iter()
                .any(|r| r.name == "Run" && r.receiver_hint == "svc"),
            "refs: {:?}",
            f.references
        );
        assert!(
            f.references
                .iter()
                .any(|r| r.name == "Create" && r.receiver_hint == "Service"),
            "refs: {:?}",
            f.references
        );
    }
    ...
    fn import_module_captured() {
        ...
        assert!(
            f.imports
                .iter()
                .any(|i| i.path == "Foo" && i.kind == "import-module"),
            "imports: {:?}",
            f.imports
        );
    }
    ...
    fn using_namespace_captured() {
        ...
        assert!(
            f.imports
                .iter()
                .any(|i| i.path == "System.IO" && i.kind == "using-namespace"),
            "imports: {:?}",
            f.imports
        );
    }
