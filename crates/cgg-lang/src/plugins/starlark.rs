...
impl LanguagePlugin for StarlarkPlugin {
    fn id(&self) -> &'static str { "starlark" }
    fn extensions(&self) -> &'static [&'static str] { &[".bzl", ".star", ".bazel"] }
    fn ts_language(&self) -> tree_sitter::Language { tree_sitter_starlark::LANGUAGE.into() }
    ...
    fn extract(&self, file: FileId, path: &Path, tree: &Tree, source: &[u8]) -> FileFacts {
        ...
        let mut w = StarlarkWalker { source, facts: &mut facts };
        w.walk(tree.root_node());
        facts
    }
}

struct StarlarkWalker<'a> { source: &'a [u8], facts: &'a mut FileFacts }
...
impl<'a> StarlarkWalker<'a> {
    ...
    fn walk(&mut self, node: Node) {
        match node.kind() {
            "function_definition" => { self.record_function(node); self.walk_children(node); return; }
            "call" => { self.record_call(node); self.walk_children(node); return; }
            _ => {}
        }
    ...
    fn record_call(&mut self, node: Node) {
        ...
        let (name, receiver) = match callee.kind() {
            ...
            "attribute" => {
                ...
                let object = callee.child(0).map(|n| self.text(n).to_string()).unwrap_or_default();
                let last = callee.children(&mut callee.walk())
                    .filter(|c| c.kind() == "identifier").last();
        ...
        if name == "load" && receiver.is_empty() {
            if let Some(args) = node.child(1) {
                if let Some(first_str) = args.children(&mut args.walk())
                    .find(|c| c.kind() == "string")
                {
                    ...
                    if !path.is_empty() {
                        self.facts.imports.push(ImportRecord {
                            kind: "load".into(), path, alias: String::new(),
...
mod tests {
    ...
    fn extract(src: &str) -> FileFacts {
        ...
        p.set_language(&tree_sitter_starlark::LANGUAGE.into()).unwrap();
        let tree = p.parse(src, None).unwrap();
        StarlarkPlugin.extract(FileId::new(0), &PathBuf::from("/tmp/x.bzl"), &tree, src.as_bytes())
    }
    ...
    fn load_captured_as_import() {
        ...
        let f = extract(src);
        assert!(f.imports.iter().any(|i| i.path == "//common:macros.bzl" && i.kind == "load"),
            "imports: {:?}", f.imports);
    }
    ...
    fn attribute_call() {
        ...
        let f = extract(src);
        assert!(f.references.iter().any(|r| r.name == "cc_library" && r.receiver_hint == "native"),
            "refs: {:?}", f.references);
    }
}
