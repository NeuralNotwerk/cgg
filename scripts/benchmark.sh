...
APPS=(
    ...
    "app-carvera-kivy|https://github.com/Carvera-Community/Carvera_Controller.git|kivy,~py-threading,~uikit-lifecycle"
...
APPS_UNVERIFIED=(
    "martini|detected, enumerates nothing. martini-contrib/render registers 21 routes as `m.Get(\"/x\", func(...) { ... })`; cgg records a value reference only for a bare identifier argument, so an inline closure handler binds to no callable. Not a missing application — Go registrars need inline-closure capture first"
    "powershell-dsc|no project found. dsccommunity/ComputeManagementDsc 404s, and nothing in the corpus declares a DSC resource — a class with [DscResource()] and Get/Set/Test-TargetResource, or a .schema.mof. Needs a repo, not a rule change"
)
