...
if have npx; then
    ...
    gate "markdownlint (all docs)" npx --yes markdownlint-cli2 $MD_FILES
else
    warn "npx not available — markdown lint skipped"
fi

if have ruff; then
    # Lint AND format. `ruff check` alone let 9 files drift out of format
    # while reporting clean, because they are different questions.
    gate "ruff lint"   ruff check  scripts/ crates/cgg-py/tests/
    gate "ruff format" ruff format --check scripts/ crates/cgg-py/tests/
else
    warn "ruff not installed — python lint/format skipped"
fi

# The hand-written JavaScript. `index.js`/`index.d.ts` are generated by the
# napi CLI and deliberately excluded: formatting a generated file only
# guarantees a fight with its generator on the next build.
if have node; then
    gate "node syntax (tests)" sh -c 'for f in crates/cgg-node/tests/*.js; do node --check "$f" || exit 1; done'
else
    warn "node not available — JS syntax check skipped"
fi

# TOML is the release surface most likely to be edited by hand under time
# pressure, and a malformed manifest fails late and confusingly.
gate "toml parses" python3 -c '
import sys, tomllib, pathlib
bad = []
for p in pathlib.Path(".").glob("**/Cargo.toml"):
    if "target" in p.parts or "node_modules" in p.parts: continue
    try: tomllib.loads(p.read_text())
    except Exception as e: bad.append(f"{p}: {e}")
for p in pathlib.Path(".").glob("*.toml"):
    try: tomllib.loads(p.read_text())
    except Exception as e: bad.append(f"{p}: {e}")
if bad:
    print("\n".join(bad)); sys.exit(1)
